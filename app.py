...
with st.sidebar:
    ...
    if page == "Interactive Portfolio Optimizer":
        num_assets = st.slider("Number of Assets", min_value=3, max_value=20, value=10, step=1)
        risk_free_rate = st.slider("Risk-Free Rate (%)", min_value=0.0, max_value=5.0, value=1.0, step=0.1) / 100
...
if page == "Introduction":
...
elif page == "Interactive Portfolio Optimizer":
    ...
    with st.spinner("Generating sample data and optimizing portfolio..."):
        returns_df, corr_matrix = generate_sample_data(num_assets=num_assets)
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Efficient Frontier", "Correlation Matrix", "Optimal Weights", "Asset Statistics"])
